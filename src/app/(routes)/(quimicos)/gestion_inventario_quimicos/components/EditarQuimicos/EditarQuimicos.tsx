...
import { quimicosSchema } from "./QuimicosForm.form"
import { z } from "zod"
...
type FormValues = z.infer<typeof quimicosSchema>
...
export function EditarQuimicos({ quimico, ubicaciones, onSuccess }: Props) {
  const { data: session } = useSession()

  const form = useForm<FormValues>({
    resolver: zodResolver(quimicosSchema),
    defaultValues: {
      codigo: 0,
      descripcion: "",
      noLote: "",
  ...
  useEffect(() => {
    if (quimico) {
      form.reset({
        ...quimico,
        fechaIngreso: new Date(quimico.fechaIngreso).toISOString().split("T")[0]
      })
    }
  }, [quimico])
  ...
  return (
    <Form {...form}>
      ...
      >
        {["codigo", "descripcion", "noLote", "proveedores", "cantidad"].map((name) => (
            <FormField
                key={name}
